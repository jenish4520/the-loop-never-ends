...
public class GameLogic {
    ...
    private boolean pieceHits(Tetromino piece, int px, int py) {
        ...
        for (int r = 0; r < shape.length; r++) {
            for (int c = 0; c < shape[r].length; c++) {
                if (shape[r][c] == 1 && piece.x + c == px && piece.y + r == py) {
                    return true;
                }
            }
        }
    ...
    private int[] impactCell(Tetromino piece) {
        ...
        for (int r = 0; r < shape.length; r++) {
            for (int c = 0; c < shape[r].length; c++) {
                if (shape[r][c] == 1) return new int[]{piece.x + c, piece.y + r};
            }
        }
    ...
    public void hardDrop(PlayerState p) {
        if (p.isGameOver) return;
        if (hardDropPiece(p, false)) lockPiece(p);
        if (twoBlocksMode && p.activePiece2 != null && hardDropPiece(p, true)) lockPiece2(p);
    }
