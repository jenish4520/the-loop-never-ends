...
import seda_project.control_alt_defeat.chess.ChessApp;
...
public class GameHub {
    ...
    public void show() {
        ...
        VBox tetrisCard = buildModeCard("2.", "TETRIS", "Block Puzzle", ACCENT_PINK, () -> {
            TetrisApp tetrisGame = new TetrisApp(stage, this);
            tetrisGame.show();
        });

        VBox chessCard = buildModeCard("3.", "CHESS", "Board Game", ACCENT_PURPLE, () -> {
            ChessApp chessGame = new ChessApp(stage, this);
            chessGame.show();
        });

        cardsRow.getChildren().addAll(memoryCard, tetrisCard, chessCard);
